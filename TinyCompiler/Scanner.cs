    ...
    public class Scanner
    {
        ...
        public void StartScanner(string code)
        {
            for (int i = 0; i < code.Length; i++)
            {
                ...
                // String ".*"
                else if (code[i] == '"') // "asdsd,nbjk/wqe2131qwew!@#asdwds"
                {
                    bool gotERR = false;
                    data += code[i]; // "
                    i++;
                ...
                {
                        while ((code[i] >= '0' && code[i] <= '9') || code[i] == '.' || (code[i] >= 'A' && code[i] <= 'z'))//|| (code[i] >= 'A' && code[i] <= 'z')) // TODO: handel 2a error
                        {
                            data += code[i];
                            i++;
                            if (i >= code.Length)
        ...
        // Getting Token
        public void getToken(string LEX) { // "asdasda2!23"
            bool isUndefined = false;
            Token tkn = new Token();
            tkn.lex = LEX; 

            // Is Reserved WORD ?
            if (ReservedKeys.ContainsKey(LEX)) // return
            ...
            // Is Float ?
            else if (isFloat(LEX)|| isNumber(LEX))
